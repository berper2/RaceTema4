...
```{r}
by_package %>%
  summarise(count = n(), unique = n_distinct(ip_id), 
...
# group_by(package) %>%
# summarise(count = n()) %>%
# quantile(count, probs = 0.99)
#Da error, paso al siguiente
...
students2 %>%
  pivot_longer(-grade, names_to = c("sex","class"),values_to = "count",
               names_sep = "_")
